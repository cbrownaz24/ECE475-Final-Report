...
{\Large ECE475: Computer Architecture, Term Project, Spring 2026}}

\author{\IEEEauthorblockN{Max Machado}
\IEEEauthorblockA{\textit{Department of Electrical and Computer Engineering} \\
\textit{Princeton University}\\
\texttt{max.machado@princeton.edu} \\
}
\and
\IEEEauthorblockN{Connor Brown}
\IEEEauthorblockA{\textit{Department of Electrical and Computer Engineering} \\
\textit{Princeton University}\\
\texttt{connorbrown@princeton.edu} \\
}
}
...
five-stage RISC-V processor developed in previous ECE475 labs. The unit
...
built across previous course labs. We then build a second variant
...
many lanes, scratchpad memory) put them out for this project, but the basic abstraction of having a single instruction
...
\begin{figure}[t]
\centerline{\includegraphics[width=\columnwidth]{media/system-arch.png}}
...
\begin{figure}[t]
\centerline{\includegraphics[width=\columnwidth]{media/vecunit-internals.png}}
...
\begin{figure}[t]
\centerline{\includegraphics[width=\columnwidth]{media/chaining.png}}
...
We add six kernels to the existing the benchmark harness from previous labs, with the
...
\item \textbf{saxpy} --- short AXPY: \texttt{y = a*x + y} on N=16
  integers. Named after the BLAS single-precision kernel; we use
  integers because our ALU is integer-only. Runs the
  VSMUL$\to$VVADD chainable producer/consumer pair without strip-mining.
\item \textbf{daxpy} --- same AXPY pattern, longer arrays
  (N $\in$ \{16, 32, 64, 128, 256\}), so N $>$ VLMAX = 32 forces the
  SETVL/VFENCE strip-mine loop. The ``D'' is borrowed from the BLAS
  double-precision kernel name; despite the name we again use 32-bit
  integers. The N-sweep lets us measure how the vector advantage
...
We borrowed the names ``saxpy'' and ``daxpy'' because the
access pattern (\texttt{y = a*x + y}) is the canonical AXPY pattern
the textbook uses to motivate chaining \cite[\S 4.2]{hp_book}. In
our setup the two kernels differ in array length and whether
strip-mining is exercised, not in numeric precision: both run on
32-bit integers, since we did not implement a floating-point unit. The reason we include both tests for distinction is primarily due to how we test strip-mining operations as well as potential extensions into floating point operations.
...
\begin{figure}[t]
\centerline{\includegraphics[width=\columnwidth]{media/speedup.png}}
...
\begin{figure}[t]
\centerline{\includegraphics[width=\columnwidth]{media/throughput.png}}
...
\begin{figure}[t]
\centerline{\includegraphics[width=\columnwidth]{media/daxpy-scaling.png}}
...
\begin{figure}[t]
\centerline{\includegraphics[width=\columnwidth]{media/linpack-scaling.png}}
...
\begin{figure}[t]
\centerline{\includegraphics[width=\columnwidth]{media/chain-depth.png}}
